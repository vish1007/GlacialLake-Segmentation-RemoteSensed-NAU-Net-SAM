...
class BottomUp(nn.Module):
    def __init__(self):
        ...
        self.pyramid_layer_1 = nn.Sequential( nn.Conv2d(in_channels=4, out_channels=64, kernel_size=7, stride=2, padding=3, bias=False),  # 4 channels
            self.resnet_layers[1],  # Original batchnorm
            self.resnet_layers[2]   # Original ReLU
        )
...
class FPNSegemer(nn.Module):
    ...
    def forward(self, input_image):
        C2, C3, C4, C5 = self.bottomup_path(input_image)
        P2, P3, P4, P5 = self.topdown_path(C2, C3, C4, C5)
